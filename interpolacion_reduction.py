...
#projection = pyproj.Proj(proj="utm",zone=19, ellps='WGS84', lat_ts=data.lat.mean())
...
projection = pyproj.Proj(proj='merc', ellps='WGS84', lat_ts=data.lat.mean())
...
# grd = vd.KNeighbors(k=5)
# grd.fit(proj_coordinates,anom_al)
# grid = grd.grid(region=region, spacing=spacing, projection=projection,dims=['lat','lon'], data_names="anom_al_mgal")

##cubic interpolation
# grd = vd.Cubic().fit(proj_coordinates,anom_al)
# grid = grd.grid(region=region, spacing=spacing, projection=projection,dims=['lat','lon'], data_names="anom_al_mgal")
...
spline.fit(proj_coordinates,anom_al)
...
plt.savefig('outputs/spline_reduced')
plt.show()